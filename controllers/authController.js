...
const registerUser = async (requisicao, res) => { 

    const { name, email, password, birth_date } = requisicao.body; // Desestrutura os dados do corpo da uisicaouisição 
    ...
    try { 
        ...
        if (existingUser.length > 0) { 
            return res.status(400).send('Usuário já registrado , tente outro email!'); 
        }
...
const loginUser = async (requisicao, res) => { 
    const { email, password } = requisicao.body; // Desestrutura os dados do corpo da uisicaouisição 
...
const requestPasswordReset = async (req, res) => { 
   ...
   const resetPassword = async (req, res) => { 
    ...
    await db.promise().query('UPDATE users SET password = ?, reset_password_token = NULL, reset_password_expires = NULL WHERE id = ?', [hashedPassword, user[0].id]); res.send('Senha redefinida com sucesso'); 
    } catch (err) { 
...
alter table users add column newPassword varchar(50);